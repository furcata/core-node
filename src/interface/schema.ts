 ...
 */
const toParseIssues = (issues: readonly z.core.$ZodIssue[]): ParseIssue[] => issues.map((issue) => ({
 ...
 */
const toMessage = (label: string, issues: ParseIssue[]): string => {
 ...
 */
export const parseResult = <TSchema extends z.ZodType>(
  schema: TSchema,
  value: unknown,
  label: string,
 ...
 */
export const requiredKey = <TSchema extends z.ZodType>(
  schema: TSchema,
  message: string,
): z.ZodCustom<z.output<TSchema>, z.output<TSchema>> => z.custom<z.output<TSchema>>(
 ...
 */
export const documentId = (): z.ZodString => z.string().min(1).max(1500);
 ...
 */
export const token = (): z.ZodString => z.string().min(1).max(512);
 ...
 */
export const finiteNumber = (): z.ZodNumber => z.number();
 ...
 */
export const nonNegativeNumber = (): z.ZodNumber => z.number().min(0);
 ...
 */
export const counter = (): z.ZodNumber => z.int().min(0);
 ...
 */
export const epochSeconds = (): z.ZodNumber => z.int().min(0).max(9_999_999_999);
 ...
 */
export const epochMillis = (): z.ZodNumber => z.int().min(1_000_000_000_000).max(9_999_999_999_999);
 ...
 */
export const timestampLike = (): z.ZodType<TimestampLike, TimestampLike> => z.custom<TimestampLike>(
  isTimestampLike,
  {error: 'Expected a Firestore timestamp with numeric seconds and nanoseconds'},
);
 ...
 */
export const auditTimestamp = (): z.ZodType<TimestampLike | Date | string | number> =>
  z.custom<TimestampLike | Date | string | number>(
    (candidate) => {
      if (isTimestampLike(candidate)) return true;
 ...
 */
export const openValue = (): z.ZodOptional<z.ZodUnknown> => z.unknown().optional();
 ...
 */
export const matchMember = <TEnum extends Record<string, string>>(
  members: TEnum,
  value: unknown,
): MemberResult<TEnum[keyof TEnum]> => {
  if (typeof value !== 'string') return {matched: false, value};
  const accepted = Object.values(members) as string[];
  if (!accepted.includes(value)) return {matched: false, value};
  return {matched: true, member: value as TEnum[keyof TEnum]};
};
 ...
 */
export const requireMember = <TEnum extends Record<string, string>>(
  members: TEnum,
  value: unknown,
  label: string,
): TEnum[keyof TEnum] => {
  const result = matchMember(members, value);
  if (result.matched) return result.member;
